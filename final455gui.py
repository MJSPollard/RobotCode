...
class Gui455:
    ...
    def __init__(self, master, socket, address, contr):
        self.contr = contr
        self.master = master
        master.title("455GUI")
       ...
       #networking stuff
        ...
        self.speak_button = tk.Button(master, text="speak", command=lambda: self.button_pressed(9)) #new shit
    ...
    #thread that listens for input from the android client
    def listenThread(self):
        while 1:
            inputString = self.sock.recv(1024).decode()
            print('Client sent:', inputString)
            if(inputString == "start"):
                inputString = ""
                self.execute_threads(1)
    ...
    def speak_window(self, val):
        self.windo = tk.Tk()
        self.windo.title("Speak")
        self.numCan = tk.Canvas(self.windo, bg="#333333", width="0", height="0")
        self.numCan.pack(side="top", fill="both", expand=True)
        label1 = tk.Label(self.windo, text="What to say?")
        label1.pack()

        self.speech1 = tk.Button(self.windo, text="Purpose", command=lambda: self.send_message_phone("1"))
        self.speech1.pack(side=tk.BOTTOM)
        self.speech2 = tk.Button(self.windo, text="Nothing here", command=lambda: self.send_message_phone("2"))
        self.speech2.pack(side=tk.BOTTOM)
        self.speech3 = tk.Button(self.windo, text="Life anymore", command=lambda: self.send_message_phone("3"))
        self.speech3.pack(side=tk.BOTTOM)
        self.speech4 = tk.Button(self.windo, text="yes, Master", command=lambda: self.send_message_phone("4"))
        self.speech4.pack(side=tk.BOTTOM)
        self.windo.mainloop()


    def send_message_phone(self, speechText):
        print(speechText)
        #self.sock.send(speechText.encode())
        self.command_list.append([9, speechText, 9])
        currPlace = len(self.command_list)
        self.myCan.itemconfig(self.stringName[currPlace], text=str(currPlace) + ". > " + "Speaking: Type " + str(speechText))
        self.windo.quit()
        self.windo.destroy()
    ...
    def command_thread(self):
        ...
        #        time.sleep(1)
            ...
            elif (moveType == 9): #speaking
                self.sock.send(valueGiven.encode())
                while 1:
                    if(self.moveOn == True):
                        self.moveOn = False
                        break
...
if __name__ == '__main__':
    ...
    gui = Gui455(root, clientsocket, address, cont)
    root.mainloop()
